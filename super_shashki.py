...
def create_move(m_board, c_board):
    print("\n", "create move", "="*30)
    print("board")
    for i in board:
        print(i)
    ...
    for x in range(8):
        board[7][x] = 4 if board[7][x] == 3 else board[7][x]
    ...
    if m_board != None:
        ...
        m_buf = [[m_board[j][i] for i in range(8)] for j in range(8)]

    else:
        m_buf = [[c_board[j][i] for i in range(8)] for j in range(8)]

    hod = superai2.hod_kompjutera(m_buf)
    return hod

def update_memory_board(hod):
    ...
    for i in hod:
        if i[0] == 7:
            board[hod[0][0]][7-hod[0][1]] = 4

    if hod[0] != hod[len(hod)-1]:
        board[hod[len(hod) - 1][0]][7-hod[len(hod) - 1][1]] = board[hod[0][0]][7-hod[0][1]]
        board[hod[0][0]][7-hod[0][1]] = 0
...
def move():
    ...
    #Создать ход исходя из доски с памятью дамок и нынешней доски!!!
    move = create_move(memory_board, board)

    #Обновить доск с памятью дамок
    update_memory_board(move)
